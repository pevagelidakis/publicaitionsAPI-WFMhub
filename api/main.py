...
.search-box input[type="text"] {{
    ...
    border: 2px solid #ccc; border-radius: 30px 0 0 30px;
}}
.search-box button {{
    padding: 12px 25px; font-size: 16px;
    border: 2px solid #4A90E2; border-left: none;
    background-color: #4A90E2; color: white;
    border-radius: 0 30px 30px 0; cursor: pointer;
}}
...
"""
    ...
    if search_query:
        html_content += f"""
<h2 style="text-align:center;">Latest Papers for "{safe_query}"</h2>
"""
        for p in papers:
            html_content += f"""
<div class="paper-section">
    <div class="authors">
        <a href="{esc(p['URL'])}" target="_blank">{esc(p['Authors'])}</a>
    </div>
    <div class="title">
        <a href="{esc(p['URL'])}" target="_blank">{esc(p['Title'])}</a>
    </div>
    <p class="authors">
        arXiv: {esc(p['DOI'])} — {esc(p['Published'])}
    </p>
    <p class="abstract">
        <b>Abstract.</b> {esc(p['Abstract'])}
    </p>
</div>
"""
    else:
...
def papers(query: str = Query(default="", max_length=300)):
    query = query.strip()
    results = get_arxiv_full_metadata(query, MAX_PAPERS) if query else []
